...
# y_neg = relu(conv(x, fp, s, p)) - relu(conv(x, fn, s, p))

y_neg = relu(conv(xp, fp, s, p)) - relu(conv(xn, fp, s, p)) - relu(conv(xp, fn, s, p)) + relu(conv(xn, fn, s, p))

'''
y_ref has no negative output
why would we expect to be equivalent if we want negative output lol.

we will need to experiment with 2 conv layers then ??
yes i think so 
'''
