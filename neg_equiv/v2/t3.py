...
tw3 = tf.get_variable("tw3", [3,3,f2,f3], dtype=tf.float32)
...
tconv3 = tf.nn.relu(tf.nn.conv2d(tpool2, tw3, [1,1,1,1], 'SAME'))

####################################

loss = tf.losses.mean_squared_error(labels=tconv3, predictions=conv3)
params = [w1p, w1n, w2p, w2n, w3]
grads = tf.gradients(loss, params)
grads_and_vars = zip(grads, params)
train = tf.train.AdamOptimizer(learning_rate=lr, epsilon=1.).apply_gradients(grads_and_vars)
...
for ii in range(50):
    ...
    for jj in range(0, 50000, 50):
        ...
        xs = x_train[s:e]
        ys = y_train[s:e]
        [l, o, to, _] = sess.run([loss, conv3, tconv3, train], feed_dict={x: xs, y: ys, lr: np.power(1.05, ii)})
    ...
    print ('lr: %f | loss %f | match %f | nonzero %f %f | weight match %f %f' % (np.power(1.05, ii), np.average(losses), np.average(sign_matches), np.average(nonzeros), np.average(nonzeros_target), sign_match_w1, sign_match_w2))
