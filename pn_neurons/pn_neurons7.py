...
mask3 = tf.constant(np.array([1.] * 128 + [-1.] * 128), dtype=tf.float32)
mask2 = tf.constant(np.array([1.] * 96 + [-1.] * 96), dtype=tf.float32)
mask1 = tf.constant(np.array([1.] * 64 + [-1.] * 64), dtype=tf.float32)
...
dact3  = dpool3 * tf.cast(tf.abs(conv3) > 0.0, dtype=tf.float32) * mask3
dconv3 = tf.nn.conv2d_backprop_input(input_sizes=[1,8,8,192], filter=tf.ones_like(tf.concat([w3[0], w3[1]], axis=3)), out_backprop=dact3, strides=[1,1,1,1], padding='SAME')

dpool2 = gen_nn_ops.avg_pool_grad(orig_input_shape=[1,16,16,192], grad=dconv3, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
dact2  = dpool2 * tf.cast(tf.abs(conv2) > 0.0, dtype=tf.float32) * mask2
dconv2 = tf.nn.conv2d_backprop_input(input_sizes=[1,16,16,128], filter=tf.ones_like(tf.concat([w2[0], w2[1]], axis=3)), out_backprop=dact2, strides=[1,1,1,1], padding='SAME')

dpool1 = gen_nn_ops.avg_pool_grad(orig_input_shape=[1,32,32,128], grad=dconv2, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
dact1  = dpool1 * tf.cast(tf.abs(conv1) > 0.0, dtype=tf.float32) * mask1
dconv1 = tf.nn.conv2d_backprop_input(input_sizes=[1,32,32,6], filter=tf.ones_like(tf.concat([w1[0], w1[1]], axis=3)), out_backprop=dact1, strides=[1,1,1,1], padding='SAME')
...
print (np.sum(np.sign(grad) == np.sign(dpool1)) * 1.0 / np.prod(np.shape(grad)))
